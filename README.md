...
3. **Prior-knowledge screening** - all 30 questions shown; participants tick the ones they already know. Knowing 11+ excludes them (too little new material to learn). Eligible participants are then assigned a group and given 20 questions drawn from the ones they did _not_ already know.
...
- Group assignment alternates strictly (A, B, A, B…) via a Supabase RPC, assigned only _after_ a participant passes screening, so the groups stay balanced.
